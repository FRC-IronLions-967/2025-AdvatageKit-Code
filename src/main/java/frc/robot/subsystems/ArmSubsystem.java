...
public class ArmSubsystem extends SubsystemBase {
  ...
  public ArmSubsystem() {
    ...
    elevatorVortexConfig.inverted(true).smartCurrentLimit(60).idleMode(IdleMode.kBrake);
    elevatorVortexConfig
        .encoder
        .positionConversionFactor(
            (2.0 * 2.0 * Constants.elevatorSprocketRadius * Math.PI)
                / Constants.elevatorGearRatio) // to meters (doubled to account for cascade rigging)
        .velocityConversionFactor(
            (2.0 * 2.0 * Constants.elevatorSprocketRadius * Math.PI)
                / (60.0 * Constants.elevatorGearRatio)); // to meters/sec
    elevatorVortexConfig
        .analogSensor
    ...
    elevatorVortex.configure(
        elevatorVortexConfig,
        SparkBase.ResetMode.kResetSafeParameters,
        SparkBase.PersistMode.kPersistParameters);

    armVortexConfig.smartCurrentLimit(40).idleMode(IdleMode.kBrake);
    armVortexConfig
        .absoluteEncoder
    ...
    armVortexConfig
        ...
        .positionWrappingInputRange(
            Constants.armWiringMinConstraint, Constants.armWiringMaxConstraint)
    ...
    coralManipulatorVortexConfig.smartCurrentLimit(60).idleMode(IdleMode.kCoast);
    coralManipulatorVortexConfig
        .closedLoop
        .feedbackSensor(FeedbackSensor.kPrimaryEncoder)
        .pid(1e-4, 0, 0);

    coralManipulatorVortex.configure(
        coralManipulatorVortexConfig,
        SparkBase.ResetMode.kResetSafeParameters,
        SparkBase.PersistMode.kPersistParameters);

    algaeManipulatorVortexConfig.smartCurrentLimit(60).idleMode(IdleMode.kCoast);
    algaeManipulatorVortexConfig
        .closedLoop
        .feedbackSensor(FeedbackSensor.kPrimaryEncoder)
        .pid(1e-4, 0, 0);

    algaeManipulatorVortex.configure(
        algaeManipulatorVortexConfig,
        SparkBase.ResetMode.kResetSafeParameters,
        SparkBase.PersistMode.kPersistParameters);
  ...
  public boolean isArmInPosition() {
    return getArmAngle() - Constants.armTolerance < rotaryArmEndGoal
        && rotaryArmEndGoal < getArmAngle() + Constants.armTolerance;
  }
  ...
  private void setArmState() {
    ...
    if (elevatorHomed
        && (state != ArmStates.STARTUP
            || getElevatorPosition() >= Constants.armFullRotationElevatorHeight)) {
      if (hasAlgae()) {
  ...
  public void periodic() {
    ...
    switch (state) {
      ...
      case STARTUP:
        elevatorHeightEndGoal =
            Constants.armFullRotationElevatorHeight + Constants.elevatorTolerance;
      case EMPTY:

        // Makes Rotation Safe
        if (getElevatorPosition()
            <= Constants.armFullRotationElevatorHeight
                + Constants.armTolerance) { // Fudge factor for imperfect positioning
          rotaryArmCurrentTarget =
              (rotaryArmEndGoal >= Constants.emptyArmConstraintForAlgaeManipulatorAtE0)
                  ? Constants.emptyArmConstraintForAlgaeManipulatorAtE0
                  : rotaryArmEndGoal;
        } else {
        ...
        // Makes Elevator Safe
        if (elevatorHeightEndGoal < getElevatorPosition()
            && elevatorHeightEndGoal < Constants.armFullRotationElevatorHeight
            && getArmAngle() >= Constants.emptyArmConstraintForAlgaeManipulatorAtE0) {
      ...
      case ALGAE_IN:
        ...
        if (getElevatorPosition() <= Constants.armWithAlgaeFullRotationElevatorHeight) {
          ...
          rotaryArmCurrentTarget =
              (rotaryArmEndGoal >= Constants.armWithAlgaeMaxConstraint)
                  ? Constants.armWithAlgaeMaxConstraint
                  : rotaryArmEndGoal;
        }
        ...
        if (elevatorHeightEndGoal < getElevatorPosition()
            ...
            && (Constants.armWithAlgaeMinConstraint <= getArmAngle()
                && getArmAngle() <= Constants.armWithAlgaeMaxConstraint)) {
          elevatorHeightCurrentTarget = Constants.armWithAlgaeFullRotationElevatorHeight;
        } else {
          elevatorHeightCurrentTarget = elevatorHeightEndGoal;
        }
    ...
    // elevatorVortexController.setReference(elevatorHeightCurrentTarget +
    // Constants.kElevatorAnalogZeroOffset, ControlType.kPosition, ClosedLoopSlot.kSlot0, 0.09,
    // ArbFFUnits.kPercentOut);
    elevatorVortexController.setReference(
        elevatorHeightCurrentTarget,
  ...
  public void simulationPeriodic() {
    ...
    armVortexSim.iterate(-armSim.getVelocityRadPerSec(), 12.0, Robot.defaultPeriodSecs);
